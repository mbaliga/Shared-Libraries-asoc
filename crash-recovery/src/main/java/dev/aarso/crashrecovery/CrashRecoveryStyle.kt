 ...
 * any design system's token type, so a design-system consumer and a non-design-system app
 * (Animalcules, Clackpad -- see D-L) can both pass plain `@ColorInt Int`s without taking a
 * dependency on anything beyond this module. Light vs dark is resolved at display time from
 * the device's night setting (see [resolve]).
 */
