 ...
 * reliability utility, not a design-system dependency -- it has no dependency on any
 * design-system module and imposes no visual language, so apps with their own visual
 * identity (that must never depend on a shared design system) can still take this one
 * dependency (see Personal-Tracker DECISIONS.md D-O).
 *
